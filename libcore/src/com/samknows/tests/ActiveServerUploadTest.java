...
public final class ActiveServerUploadTest extends UploadTest {
  ...
  private String formPostHeaderRequest(int numThreads, int threadIndex) {
    ...
    long millisecondsWarmupSampleTime = (long) (getWarmupMaxTimeMicro() / 1000.0);
  ...
  final protected boolean transfer(Socket socket, int threadIndex) {
      ...
      public void callOnStopOrCancel(String responseString, int responseCode) {
        ...
        if ((responseCode != 100) && (responseCode != 200)) {
          ...
          if (bGotValidResponseFromServer == true) {
            ...
            bitrateMpbs1024Based = Conversions.sConvertBytesPerSecondToMbps1024Based(transferBytesPerSecond);
    ...
    try {
      while (!readThread.getSemaphoreState()) {
      }													/* Wait until input thread issues OK to "proceed" */
...
//		long waitFromTime = 0;
    ...
    waitUntilTime = currentTime + getTransferMaxTimeMicro() + 1;
...
//						" thread: " + threadIndex);//haha remove in production
      ...
      if (getTransferMaxBytes() > 0) {
        if (super.getTotalWarmUpBytes() + super.getTotalTransferBytes() > getTransferMaxBytes()) {					/* we transferred more bytes that was initially allowed */
          isTransferDone = true;
        }
      }
